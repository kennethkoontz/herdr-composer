...
export function closePopup() {
  ...
  // Blocking child so the write completes before process.exit in the parent.
  const script = [
    'const net = require("net");',
    "const path = process.env.HERDR_SOCKET_PATH;",
    "if (!path) process.exit(0);",
    "const client = net.createConnection(path);",
    'const payload = JSON.stringify({id:"quick-launch:popup-close",method:"popup.close",params:{}}) + "\n";',
    "const done = () => { try { client.destroy(); } catch {} process.exit(0); };",
    "client.setTimeout(250, done);",
