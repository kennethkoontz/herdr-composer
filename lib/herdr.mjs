...
/** Pull `error.code` out of a herdr CLI failure body so callers can branch on it. */
function herdrErrorCode(text) {
  try {
...
function parseHerdrOutput(stdout, stderr, status, { allowFail = false } = {}) {
  ...
  if (status !== 0 && !allowFail) {
    const body = err || out || `herdr exited ${status}`;
    const error = new Error(body);
    error.code = herdrErrorCode(err) || herdrErrorCode(out);
    throw error;
  }
...
/**
 * Visible terminal text for an agent pane. Plain text, not JSON — used to spot
 * startup dialogs (folder trust, first-run pickers) that would otherwise eat a
 * submitted prompt.
 */
export function readAgentScreen(target, { lines = 80 } = {}) {
  const result = spawnSync(
    herdrBin(),
    ["agent", "read", target, "--source", "visible", "--lines", String(lines)],
    {
      encoding: "utf8",
      stdio: ["ignore", "pipe", "pipe"],
      env: process.env,
...
/** Detection detail for an agent (matched rule, blocker visibility, state). */
export function explainAgent(target) {
  try {
    return resultOf(
      runHerdr(["agent", "explain", target, "--json"], { allowFail: true }),
    );
  } catch {
    return null;
  }
...
export function sendKeysAgent(target, ...keys) {
  return runHerdr(["agent", "send-keys", target, ...keys], { allowFail: true });
}

export function agentStatus(target) {
  const result = resultOf(runHerdr(["agent", "get", target], { allowFail: true }));
  return result?.agent?.agent_status || null;
}
