...
export async function runComposer({
  ...
  function renderForm(out, { rows, padX, inner }) {
    ...
    // Real block caret: reverse-video the cell under the insertion point so
    // mid-line movement does not shove remaining characters to the right.
    ...
    for (let i = 0; i < bodyRows; i += 1) {
      ...
      if (segments && i < segments.length) {
        ...
        let text = seg.text;
        if (promptActive) {
          // Caret on this line when cursor is in [start, nextStart), or at
          ...
          if (onLine) {
            ...
            if (col < text.length) {
              // Cover the char at the insertion point (no extra glyph).
              text = `${text.slice(0, col)}${INVERSE}${text[col]}${RESET}${text.slice(col + 1)}`;
            } else {
              // Past last char on this line (EOF or wrap gap).
              text = `${text}${endCaret}`;
            }
          }
        }
  ...
  return new Promise((resolvePromise, reject) => {
    ...
    const onKey = (str, key) => {
      try {
        ...
        // ctrl-v: paste a clipboard image — saved to a temp PNG, its path
        // inserted at the caret so the harness can read the file. Terminal
        // text paste arrives as plain keypresses and needs no handling here.
        if (key.ctrl && key.name === "v") {
          const imagePath = saveClipboardImage();
          if (!imagePath) {
            state.error = "No image on clipboard";
            render();
            return;
          }
          if (fields[state.field] !== "prompt") {
