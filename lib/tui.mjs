...
function chip(active, icon, label) {
...
export async function runComposer({
  ...
  function renderForm(out, { rows, padX, inner }) {
    ...
    const spaceChip =
      field === "space"
        ? `${INVERSE}${BOLD} ${spaceLabel} ▾ ${RESET}`
        : `${BOLD}${WHITE}${spaceLabel}${RESET}${DIM} ▾${RESET}`;
    ...
    const modelChip = chip(field === "model", "✦", shortModel(state.model));
    const effortChip = chip(field === "effort", "⚡", state.effort || "effort");
